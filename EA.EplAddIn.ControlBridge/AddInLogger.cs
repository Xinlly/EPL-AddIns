...
/// （…\EA.EplAddIn\&lt;版本&gt;\ControlBridge\）；取不到或不可写时回退
...
/// 本类为 EA.EplAddIn.ControlBridge 私有副本（复制自 EA.EplAddIn.TextBatchEdit，不共享代码）。
/// </summary>
public static class AddInLogger
{
    ...
    private static string ResolveLogDirectory()
    {
        ...
        // 4) 临时目录
        var fallback = Path.Combine(Path.GetTempPath(), "EA.EplAddIn.ControlBridge", "logs");
    ...
    private static string? TryComputeDynamicPath(out string note)
    {
        ...
        try
        {
            ...
            var dir = Path.Combine(baseDir, "EA.EplAddIn", ver, "ControlBridge");
