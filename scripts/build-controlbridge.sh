...
#   current time; the .temp/ControlBridge.DynamicVersion.props written below is kept
#   for traceability only and is NO LONGER IMPORTED by the csproj.
# - Fixed main/tag builds: committed EA.EplAddIn.ControlBridge/release-version.props takes precedence.
...
PROJECT_REL="EA.EplAddIn.ControlBridge/EA.EplAddIn.ControlBridge.csproj"
RELEASE_PROPS="$ROOT/EA.EplAddIn.ControlBridge/release-version.props"
DYNAMIC_PROPS="$ROOT/.temp/ControlBridge.DynamicVersion.props"
...
PROPS
  ...
  EXTRA_ARGS=()
  [[ "$FORCE_DYNAMIC" -eq 1 ]] && \
    EXTRA_ARGS+=("-p:ReleaseVersionFile=$(wslpath -w "$ROOT/.temp/__no-release-controlbridge.props")")
fi
